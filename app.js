...
const SALARY_RATES = {
    "Giám đốc": 20000, "Phó Giám đốc": 20000, "Trợ lý": 18000,
    "Thư ký": 16000, "Đội Trưởng": 17000, "Đội Phó": 15000,
    "Cảnh sát viên": 10000, "Thực Tập": 5000
};
...
// ==================== CHỈNH QUYỀN (ROLE) ====================
app.post('/admin/update-role', requireAdmin, (req, res) => {
    try {
        const db = loadDB();
        const { userId, role } = req.body;

        if (!userId || !['user', 'admin'].includes(role)) {
            return res.redirect('/admin?error=Quyền không hợp lệ');
        }

        const user = db.users.find(u => u.id == userId);
        if (!user) {
            return res.redirect('/admin?error=Không tìm thấy nhân sự');
        }

        user.role = role;
        saveDB(db);

        addLog(req.session.user.username || 'Admin', 'CẬP NHẬT QUYỀN', user.displayName, `Đổi thành ${role}`);

        res.redirect('/admin?success=Đã cập nhật quyền thành công');
    } catch (err) {
        console.error(err);
        res.redirect('/admin?error=Lỗi server');
    }
});
...
app.get("/attendance", requireAuth, (req, res) => {
  ...
  user.salaryRate = Number(user.salaryRate) || 10000;
  ...
  const remainingHoursToday = Math.max(0, 100 - completedHoursToday);
  ...
  res.render('attendance', {
    ...
    error: req.query.error === 'max_hours' ? 'Bạn đã đủ giờ làm việc hôm nay!' : null
  });
});
...
app.post("/attendance/check", requireAuth, (req, res) => {
  ...
  if (activeSession) {
    ...
    if (elapsedHours < 1) {
      ...
      const remainingToday = Math.max(0, 100 - completedHoursToday);
      ...
      activeSession.status = newTotalToday >= 100 ? "Đủ giờ hôm nay" : "Hoàn thành ca";
...
app.post("/admin/update-position", (req, res) => {
  ...
  user.position = newPosition;
  // --- ĐOẠN CODE FIX LỖI KẾT NỐI ---
app.get('/get-history', (req, res) => {
    const userId = req.query.id;
    
    // Tìm nhân sự trong database của bạn
    const user = db.users.find(u => u.id == userId);

    if (!user) {
        return res.status(404).json({ message: "Không tìm thấy sĩ quan" });
    }

    // Trả về dữ liệu (Đây là ví dụ, bạn có thể thay bằng dữ liệu thật từ db.attendance)
    res.json({
        history: user.history || [], // Lấy lịch sử từ object user
        totalHours: user.thisMonth?.hours || 0,
        rate: 5000, // Hệ số lương giả định
        totalSalary: user.thisMonth?.salary || 0
    });
});
...
// Thêm API xử lý xóa vĩnh viễn
app.delete('/api/delete-attendance', async (req, res) => {
    const { userId, recordId } = req.body;

    try {
        // 1. Lệnh xóa trong Database (Ví dụ dùng MongoDB)
        // Tìm Sĩ quan theo ID và xóa bản ghi chấm công có ID tương ứng trong mảng history
        ...
        console.error(error);
        res.status(500).json({ success: false, message: "Lỗi hệ thống khi xóa Database" });
    }
});
// Đoạn mã Backend cần thiết
app.get('/get-history', (req, res) => {
    const userId = req.query.id;
    // Tìm nhân viên trong dữ liệu của bạn (ví dụ db.users)
    const user = db.users.find(u => u.id == userId);
    ...
    // Trả về dữ liệu lịch sử (cần khớp với cấu trúc bảng trong file Payroll)
    res.json({
        history: user.history || [], 
        totalHours: user.thisMonth?.hours || 0,
        rate: 50000, // Ví dụ: 50k/giờ
        totalSalary: user.thisMonth?.salary || 0
    });
});
// ĐOẠN CODE NÀY DÁN VÀO FILE BACKEND (app.js hoặc server.js)
app.get('/get-history', async (req, res) => {
    try {
        const userId = req.query.id; // Lấy ID từ ?id=1
        
        // GIẢ SỬ: Đây là nơi bạn lấy dữ liệu từ Database (MongoDB/SQL)
        // Nếu bạn dùng mảng tạm thì thay bằng biến mảng của bạn
        // Ở đây tôi tạo dữ liệu mẫu để bạn thấy nó hiện lên
        const mockHistory = [
            { date: "2024-01-30", checkIn: "08:00", checkOut: "17:00", hours: 9, salary: 100000 },
            { date: "2024-01-31", checkIn: "09:00", checkOut: "18:00", hours: 9, salary: 100000 }
        ];
    ...
    // Thêm đoạn này vào file Server của bạn
app.post('/api/delete-attendance', async (req, res) => {
    const { userId, date } = req.body;
    
    try {
        // Lệnh này sẽ tìm Sĩ quan và rút (pull) ngày công đó ra khỏi Database vĩnh viễn
        // Giả sử bạn đang dùng MongoDB:
        await User.updateOne(
            { id: userId }, 
            { $pull: { history: { ngay: date } } } 
        );

        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ success: false, message: "Không thể xóa trong Database" });
    }
});
// Thêm đoạn này vào file server của bạn
app.post('/api/delete-attendance', async (req, res) => {
    ...
    try {
        // Tìm user theo ID và xóa bản ghi trong mảng history có ngày trùng khớp
        // Lưu ý: Thay 'User' bằng tên Model của bạn (ví dụ: Siquan, Account...)
        const result = await User.updateOne(
            { id: userId }, 
            { $pull: { 
                // history hoặc attendance tùy vào tên biến trong Schema của bạn
                history: { ngay: date } 
            }}
        );
...
client.on('messageCreate', async (message) => {
    // Log ra để xem Bot có thực sự đọc được tin nhắn không
    console.log(`Có tin nhắn mới: ${message.content}`); 
    ...
    if (message.content.startsWith('!link')) {
        ...
        if (!usernameInput) {
            return message.reply('⚠️ Nhập thiếu tên rồi! VD: !link chusau');
        }

        try {
            // Đọc file database (kiểm tra kỹ đường dẫn này)
            const dbPath = './database.json'; 
            const data = JSON.parse(fs.readFileSync(dbPath, 'utf8'));
            
            // Tìm user
            const userIndex = data.users.findIndex(u => u.username === usernameInput);

            if (userIndex === -1) {
                return message.reply(`❌ Không tìm thấy user nào tên: ${usernameInput}`);
            }

            // Gắn ID
            data.users[userIndex].discordId = message.author.id;
            
            // Ghi lại file
            fs.writeFileSync(dbPath, JSON.stringify(data, null, 2));

            message.reply(`✅ Ok con trai! Đã kết nối tài khoản **${usernameInput}** với Discord ID của con.`);
            console.log(`Đã gắn ID ${message.author.id} cho ${usernameInput}`);

        } catch (err) {
            console.error("Lỗi ghi file:", err);
            message.reply('❌ Lỗi hệ thống rồi, check lại file JSON đi.');
        }
    }
});
